...
pub enum Jbig2ErrorKind {
    ...
    InvalidTemplateIndex { index: usize, max: usize },
    InvalidCombinationOperator { operator: u8 },
    InvalidReferenceCorner { corner: u8 },
    
    // Decoding errors
    MmrDecodingFailed { reason: String },
    ArithmeticDecodingFailed { reason: String },
    HuffmanDecodingFailed { reason: String },
    InvalidRunLength { length: i32 },
    
    // Resource errors
    TooManySymbols { count: usize, max: usize },
    InfiniteLoopDetected { context: String },
    BufferOverrun { position: usize, limit: usize },
...
impl Jbig2Error {
    ...
    pub fn new(msg: &str) -> Self {
        Jbig2Error {
            kind: Jbig2ErrorKind::Other { message: msg.to_string() },
            context: None,
        }
    }
    ...
    pub fn insufficient_data(required: usize, available: usize) -> Self {
        Self {
            kind: Jbig2ErrorKind::InsufficientData { required, available },
            context: None,
        }
    }
    ...
    pub fn invalid_segment(reason: impl Into<String>) -> Self {
        Self {
            kind: Jbig2ErrorKind::InvalidSegment { reason: reason.into() },
            context: None,
        }
    }
    ...
    pub fn mmr_decoding_failed(reason: impl Into<String>) -> Self {
        Self {
            kind: Jbig2ErrorKind::MmrDecodingFailed { reason: reason.into() },
            context: None,
        }
    }
    ...
    pub fn arithmetic_decoding_failed(reason: impl Into<String>) -> Self {
        Self {
            kind: Jbig2ErrorKind::ArithmeticDecodingFailed { reason: reason.into() },
            context: None,
        }
    }
    ...
    pub fn huffman_decoding_failed(reason: impl Into<String>) -> Self {
        Self {
            kind: Jbig2ErrorKind::HuffmanDecodingFailed { reason: reason.into() },
            context: None,
        }
    }
    ...
    pub fn infinite_loop_detected(context: impl Into<String>) -> Self {
        Self {
            kind: Jbig2ErrorKind::InfiniteLoopDetected { context: context.into() },
            context: None,
        }
    }
    ...
    pub fn missing_resource(resource: impl Into<String>) -> Self {
        Self {
            kind: Jbig2ErrorKind::MissingResource { resource: resource.into() },
            context: None,
        }
    }
    ...
    pub fn unsupported_feature(feature: impl Into<String>) -> Self {
        Self {
            kind: Jbig2ErrorKind::UnsupportedFeature { feature: feature.into() },
            context: None,
        }
    }
    ...
    pub fn with_position(mut self, pos: usize) -> Self {
        self.context.get_or_insert_with(ErrorContext::default).position = Some(pos);
        self
    }
    ...
    pub fn with_segment_number(mut self, number: u32) -> Self {
        self.context.get_or_insert_with(ErrorContext::default).segment_number = Some(number);
        self
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self.kind {
            InsufficientData { required, available } => {
                write!(f, "Insufficient data (required: {} bytes, available: {} bytes)", required, available)?;
            }
            ...
            DimensionsTooLarge { width, height, max } => {
                write!(f, "Dimensions too large: {}x{} (max: {})", width, height, max)?;
            }
            ...
            BufferOverrun { position, limit } => {
                write!(f, "Buffer overrun at position {} (limit: {})", position, limit)?;
            }
        ...
        if let Some(ctx) = &self.context && (ctx.position.is_some() || ctx.segment_number.is_some() || ctx.segment_type.is_some()) {
            write!(f, " [")?;
            let mut first = true;
